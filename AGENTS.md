...
- The official `SessionPersistence` seam is the logical Session authority. Its production plugin is tiered: PostgreSQL owns metadata, semantic markers, immutable settled-Turn segments and the latest derived runtime baseline; Kafka owns the exact unfinished native suffix; Valkey is only a rebuildable projection. Canonical PostgreSQL segments plus the indexed Kafka suffix must reconstruct history exactly. Worker restore may use the DSH-owned runtime baseline plus its canonical suffix, but local gap records never enter history, export or durable storage.
